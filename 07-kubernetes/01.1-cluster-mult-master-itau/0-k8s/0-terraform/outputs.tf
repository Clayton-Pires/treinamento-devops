...
#####teste para adicionar a chave privada 

# output "output-k8s_proxy_teste" {
#   value = [
#     "k8s_proxy - ${aws_instance.k8s_proxy.private_ip} - ssh -i ~/${var.key_pair_name}.pem ubuntu@${aws_instance.k8s_proxy.public_dns} -o ServerAliveInterval=60"
#   ]
# }

# output "output-k8s_proxy_teste" {
#   value = [
#     "k8s_proxy - ${aws_instance.k8s_proxy.private_ip} - ssh -i ${var.key_pair_path}${var.key_pair_name}.pem ubuntu@${aws_instance.k8s_proxy.public_dns} -o ServerAliveInterval=60"
#   ]
# }